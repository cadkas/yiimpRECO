...
void db_update_coinds(YAAMP_DB *db)
{
  ...
  debuglog("Update coinds 002 \n");
	while((row = mysql_fetch_row(result)) != NULL)
	{
    debuglog("Update coinds 003 \n");
    ...
    debuglog("Check Coin Line 10 %s\n", coind->symbol);
		...
		if(coind->newcoind) {
			...
			if (ignore) {
				object_delete(coind);
        debuglog("Ignore Coin Line 10 %s\n", coind->symbol);
				continue;
			}
		}

    debuglog("Check Coin Line 20 %s\n", coind->symbol);
  ...
  debuglog("So terminate coinds with no touch \n");
	...
	for(CLI li = g_list_coind.first; li; li = li->next)
	{
		...
		if(!coind->touch)
		{
      debuglog("Terminating because touch is not TRUE %s\n", coind->symbol);
			coind_terminate(coind);
			continue;
		}

    debuglog("Set Touch False1 %s\n", coind->symbol);
		coind->touch = false;
	}
  debuglog("Leave loop \n");
